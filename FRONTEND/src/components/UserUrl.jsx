...
const UserUrl = () => {
  ...
  const handleDelete = async (shortUrlId) => {
    if (window.confirm("Are you sure you want to delete this URL?")) {
      setIsDeleting(true);
      setDeleteError(null);

      try {
        const result = await deleteShortUrl(shortUrlId);
        if (result.success) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {urls.urls.reverse().map((url) => (
              <tr key={url._id} className="hover:bg-gray-50">
                ...
                <td className="flex space-x-8 px-6 py-4 text-sm font-medium">
                  <button
                    onClick={() =>
                      handleCopy(
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="inline-flex items-center px-3 py-1.5  border border-transparent text-xs font-medium rounded-md shadow-sm bg-red-600 text-white hover:bg-red-700"
                  >
